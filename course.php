	...
	<div class="OneOfTwo">
		<div class="subCont">
			<ul class="course_list2">
				...
				{
					if($row['linkurl']=='' and $cfg_isreurl!='Y') $gourl = 'courseshow.php?cid='.$row['classid'].'&id='.$row['id'];
					else if($cfg_isreurl=='Y') $gourl = 'courseshow-'.$row['classid'].'-'.$row['id'].'-1.html';
					else $gourl = $row['linkurl'];

					$row2 = $dosql->GetOne("SELECT `classname` FROM `#@__infoclass` WHERE `id`=".$row['classid']);
					if($cfg_isreurl!='Y') $gourl2 = 'course.php?cid='.$row['classid'];
					else $gourl2 = 'course-'.$row['classid'].'-1.html';
				?>
